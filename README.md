...
- 楼主追记闭环：作者可为自己的帖子追加最多两条不可修改的补充说明（每条≤500字）；命中敏感词进入审核，原帖与已通过追记保持公开，同一帖子仅允许一条待审追记，驳回后可另写一条且驳回原因仅作者可见
...
| GET | /posts/{id} | 帖子详情（含楼主标记与追记） |
| GET | /posts/{id}/comments | 帖子评论 |
| POST | /posts/{id}/addenda | 楼主追记（最多两条、不可修改、每条≤500字，需登录） |
