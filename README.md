...
- 楼主追记：作者可为帖子追加最多 2 条不可修改的补充说明（每条 ≤500 字），命中敏感词进入审核且不影响原帖与已通过追记；同一帖子仅允许一条待审追记，超限或并发提交整次拒绝；驳回原因仅作者可见，驳回后可重新追记
...
| GET | /posts/{id} | 帖子详情（含楼主追记，楼主可见待审/驳回及原因） |
| POST | /posts/{id}/supplements | 楼主追加追记（需登录，最多 2 条，每条 ≤500 字） |
